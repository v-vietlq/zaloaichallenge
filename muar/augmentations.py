...
class BatchRandAugment(nn.Module):
    ...
    def __init__(self,
        ...
        if use_resize is not None:
            assert len(
                image_size) == 2, 'Invalid `image_size`. Must be a tuple of form (h, w)'
    ...
    def setup(self):
        ...
        sampled_tfms = list(np.random.choice(
            self.transform_list, self.N_TFMS, replace=False)) + [self.normalize]
...
class MuAugment():
    ...
    def __call__(self, batch):
        ...
        if len(yb.shape) == 2:
            yb = yb.squeeze(1)
        BS, TIMESTEPS, N_CHANNELS, HEIGHT, WIDTH = xb.shape
        if self.image_size is not None:
            HEIGHT, WIDTH = self.image_size
        C_images = torch.zeros(self.C, BS, TIMESTEPS,
                               N_CHANNELS, HEIGHT, WIDTH, device=self.device)

        if self.transform.mix is not None:
            C_targets = torch.zeros(
                self.C, BS, 3, device=self.device, dtype=torch.long)
        else:
            C_targets = torch.zeros(
        ...
        S_targets = S_targets.view(-1,
                                   3) if self.transform.mix else S_targets.view(-1)
