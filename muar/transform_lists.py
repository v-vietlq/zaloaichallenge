...
def albumentations_list(MAGN: int = 4):
    ...
    transform_list = [
        ...
        A.OpticalDistortion(distort_limit=M*.2,
                            shift_limit=M*.1, always_apply=True),
        A.RGBShift(r_shift_limit=M*10, g_shift_limit=M*10,
                   b_shift_limit=M*10, always_apply=True),
        A.ISONoise(color_shift=(M*.01, M*.1),
                   intensity=(M*.02, M*.2), always_apply=True),
        A.RandomFog(fog_coef_lower=M*.01,
                    fog_coef_upper=M*.1, always_apply=True),
        ...
        A.ElasticTransform(alpha=M*.25, sigma=M*3,
                           alpha_affine=M*3, always_apply=True),
...
def kornia_list(MAGN: int = 4):
    ...
    transform_list = [
        ...
        # PIXEL-LEVEL
        # brightness
        K.ColorJitter(brightness=MAGN/30, p=1),
        # saturation
        K.ColorJitter(saturation=MAGN/30, p=1),
        # contrast
        K.ColorJitter(contrast=MAGN/30, p=1),
        # hue
        K.ColorJitter(hue=MAGN/30, p=1),
        # identity
        K.ColorJitter(p=0),
        K.RandomMotionBlur(kernel_size=2*(MAGN//3)+1,
                           angle=MAGN, direction=1., p=1),
        K.RandomErasing(scale=(MAGN/100, MAGN/50),
                        ratio=(MAGN/20, MAGN), p=1),
    ]
    return transform_list
